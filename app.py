...
def excluir_leitor(id_leitor):
    ...
    if leitor:
        if leitor.emprestimos:
            flash(f'Erro: {leitor.nome} possui históricos e não pode ser excluído.', 'danger')
        else:
...
def emprestar():
    ...
    leitor_id = int(request.form.get('leitor_id'))
    leitor = Leitor.query.get(leitor_id)
    livro = Livro.query.get(livro_id)

    if Emprestimo.query.filter_by(leitor_id=leitor_id, data_devolucao=None).first():
        flash(f'Erro: {leitor.nome} já possui um livro emprestado.', 'danger')
    elif livro and livro.quantidade > 0:
        prazo = datetime.strptime(request.form.get('prazo_devolucao'), '%Y-%m-%d').date()
        db.session.add(Emprestimo(livro_id=livro.id, leitor_id=leitor.id, nome_pessoa=leitor.nome, prazo_devolucao=prazo))
        livro.quantidade -= 1
        db.session.commit()
